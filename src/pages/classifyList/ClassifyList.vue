<template lang="pug">
  ScrollView
    .topic(v-if="isKOL") 下面是{{kolname}}订阅的公众号，小新已经为您关注啦！ 每天为您推送当日的公众号更新，优化您的阅读体验～
    ...
    div(v-else)
      .kindContent
        .kindsList(v-for='(res,i) in allList' :key='index')
          ...
          .contentFlex
            .public-address-component-list(v-for='(item,index) in res.weChatPublicVOs' :key='index')
              ...
              div(class="public-address-info")
                div(class="public-address-title" v-html='item.weChatName')
                div(class="public-address-introduction") {{item.description}}
              div(v-if='item.isSubscribe !== undefined' :class="['public-address-subscribe',subscribe(item.isSubscribe)]" @click='setnormalSubscribeStatus(item,index)') 
                i(v-if='item.isSubscribe' class="iconfont iconduihao")
                div {{item.isSubscribe ? '已订阅' : '订阅'}}
...
export default class ClassifyList extends Vue {
  ...
  setnormalSubscribeStatus(item: any, index: number) {
    ...
    if (isSubscribe) {
      ...
      this.subscribenormalFnc(weChatPublicId, index)
    }
  }
  async subscribenormalFnc(weChatPublicId: number, index: number) {
    this.CanClick('')
    const res: Ajax.AxiosResponse | any = await subscribe.subscribe({
      isSearchResult: false,
      weChatPublicId: weChatPublicId
